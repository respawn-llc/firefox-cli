...
- [ ] Resolve and bound pending native messaging requests in `packages/native-host/src/native-host-runtime.ts` and `packages/extension/src/background-controller.ts`.
  Source: subagent-assisted finding, verified directly against code.
