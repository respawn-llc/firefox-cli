...
describe("disposable Firefox process cleanup parsing", () => {
  it("keeps only Firefox processes for the disposable profile", () => {
    const pids = parseDisposableFirefoxProcessIds(
      [
        "101 /Applications/Firefox.app/Contents/MacOS/firefox --profile /tmp/fc-profile",
        "102 /usr/bin/firefox-bin -profile /tmp/fc-profile",
        "103 /usr/bin/python /tmp/fc-profile",
        "104 /usr/bin/firefox --profile /tmp/other-profile",
        "105 /usr/bin/firefox-esr --profile /tmp/fc-profile",
        "106 /tmp/firefox-helper --profile /tmp/fc-profile",
      ].join("\n"),
