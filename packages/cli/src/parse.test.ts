...
describe("CLI argument parsing helpers", () => {
  it("keeps recognized options out of normal positionals", () => {
    expect(parsePositionalsAndOptions(["--window", "1", "--tab", "2", "--json", "--screenshot-format", "jpeg", "selector", "--unknown"])).toEqual({
  ...
  it("switches option-like values into payload positionals after the command payload starts", () => {
    expect(
      parsePayloadPositionalsAndOptions(["--timeout", "1000", "--json", "console.log('--literal')", "--max-output", "10"], {
