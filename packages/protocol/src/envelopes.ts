...
import { protocolErrorSchema, type Boundary, type ParseResult, type ProtocolError } from "./core.js";
import { decodeRaw, isKnownBoundary } from "./envelopes-codec.js";
import {
  createValidatedErrorResponseEnvelope,
  createValidatedOkResponseEnvelope,
  createValidatedOkResponseEnvelopeForRequest,
  createValidatedRequestEnvelope,
  createValidatedResponseEnvelopeWithVersion,
  requestCommand,
} from "./envelopes-factory.js";
import {
...
import { createRequestProtocolMismatchError, getRequestProtocolCompatibility, type RequestProtocolCompatibility } from "./protocol-compatibility.js";
...
export interface ParseBoundaryRequestOptions {
  readonly protocolVersion?: number;
  readonly hello?: HelloRequestNegotiationOptions;
}

export interface ParseBoundaryResponseOptions {
  readonly protocolVersion?: number;
...
export function parseBoundaryRequest(boundary: Boundary, raw: unknown, options: ParseBoundaryRequestOptions = {}): ParseResult<RequestEnvelope> {
  if (!isKnownBoundary(boundary)) {
  ...
  const compatibilityError = requestCompatibilityError(envelope.data.command, compatibility, expectedProtocolVersion);
  ...
  return {
    ok: true,
    value: createValidatedRequestEnvelope(envelope.data.command, params.data, envelope.data.id, expectedProtocolVersion),
  };
}
...
export function parseBoundaryResponse(
  ...
  if (envelope.data.ok) {
    ...
    return {
      ok: true,
      value: createValidatedOkResponseEnvelope(result.data, envelope.data.id, expectedProtocolVersion),
    };
  }
...
export function createErrorResponse(id: string, error: ProtocolError, protocolVersion: number = PROTOCOL_VERSION): ResponseEnvelope {
...
export function withRequestProtocolVersion<C extends CommandId>(request: RequestEnvelope<C>, protocolVersion: number): RequestEnvelope<C> {
  return {
    ...request,
    protocolVersion,
  };
...
function requestCompatibilityError(command: CommandId, compatibility: RequestProtocolCompatibility, protocolVersion: number): ParseResult<never> | undefined {
  if (compatibility.compatible) {
    return undefined;
  }

  return failure("VERSION_MISMATCH", "Request requires a newer protocol version than the negotiated session.", {
    command,
